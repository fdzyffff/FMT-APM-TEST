...
void Copter::update_land_detector()
{
    ...
    if (!motors->armed()) {
        ...
        bool rangefinder_check = true;//(!rangefinder_alt_ok() || rangefinder_state.alt_cm_filt.get() < LAND_RANGEFINDER_MIN_ALT_CM);

        if (motor_at_lower_limit && accel_stationary && descent_rate_low && rangefinder_check) {
            // landed criteria met - increment the counter and check if we've triggered
            if( land_detector_count < ((float)LAND_DETECTOR_TRIGGER_SEC)*400.f) {
                land_detector_count++;
            } else {
                set_land_complete(true);
            }
    ...
    set_land_complete_maybe(ap.land_complete || (land_detector_count >= LAND_DETECTOR_MAYBE_TRIGGER_SEC*400.f));
}
