...
private:
    ...
    float           _power_output_low = 0.0f;   // setpoint for power output at minimum rotor power  当0总距时，
    float           _power_output_high = 0.0f;  // setpoint for power output at maximum rotor power  最大总距
    float           _power_output_negc = 0.0f;  // setpoint for power output at full negative collective  最小总距
    ...
    void            update_rotor_ramp(float rotor_ramp_input, float dt);//缓启动时间，斜坡线性启动
